...
result_name = str(uuid.uuid4()) + ".txt"
...
def main():
    ...
    parser.add_argument("--run_times", default=1, help='run times')
    parser.add_argument("--seed", default=None)
    args = parser.parse_args()
    # run(args)
    infos = run(args, int(args.run_times), args.seed )
    ...
    for i in range(total):
        if 0 in infos[i].get('winners', []): 
            win_count += 1
    print("Agent 0 winning ratio: {}".format(win_count/total))
    sys.stdout = sys.__stdout__
